...
class SpectralClustering(object):
    ...
    def __init__(self, n_clusters, n_reductions = None, 
                 metric = 'rbf_kernel', models = 'ncut'):
        self.n_clusters = n_clusters
        if n_reductions == None:
            self.n_reductions = n_clusters
        else:
    ...
    def fit(self, data):
        ...
        feature_data = self.compute_k_eigenvector(laplace_matrix)
        
        labels = KMeans(n_clusters = self.n_clusters).fit(feature_data)
        return labels
